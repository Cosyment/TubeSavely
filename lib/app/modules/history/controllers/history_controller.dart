...
class HistoryController extends GetxController {
  ...
  Future<void> deleteSelected() async {
    ...
    try {
      ...
      // 保存剩余项目
      await _storageProvider.saveDownloadHistory(remainingItems);
    ...
    try {
      ...
      final filteredHistory = allHistory.where((video) {
        return video.title.toLowerCase().contains(keyword.toLowerCase()) ||
            (video.platform?.toLowerCase().contains(keyword.toLowerCase()) ??
                false) ||
            (video.author?.toLowerCase().contains(keyword.toLowerCase()) ??
                false);
      }).toList();
