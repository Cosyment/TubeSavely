...
class VideoPlayerController extends GetxController {
  ...
  void onInit() {
    ...
    // 监听播放状态
    ever(_videoPlayerRepository.status, (status) {
      this.status.value = status;
    });
    
    // 监听播放进度
    ever(_videoPlayerRepository.position, (position) {
      this.position.value = position;
    });
    
    // 监听视频总时长
    ever(_videoPlayerRepository.duration, (duration) {
      this.duration.value = duration;
    });
    
    // 监听音量
    ever(_videoPlayerRepository.volume, (volume) {
      this.volume.value = volume;
    });
    
    // 监听静音状态
    ever(_videoPlayerRepository.isMuted, (isMuted) {
      this.isMuted.value = isMuted;
    });
    
    // 监听全屏状态
    ever(_videoPlayerRepository.isFullscreen, (isFullscreen) {
